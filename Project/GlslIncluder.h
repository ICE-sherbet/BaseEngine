...
class GlslIncluder : public shaderc::CompileOptions::IncluderInterface {
 public:
  ...
  std::unordered_set<IncludeData>&& GetIncludeData() {
    return std::move(m_includeData);
  }
  std::unordered_set<std::string>&& GetParsedSpecialMacros() {
    return std::move(m_ParsedSpecialMacros);
  }
