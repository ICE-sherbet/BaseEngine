...
namespace base_engine::renderer {
enum class ShadowQualitySetting { None = 0, Low = 1, High = 2 };

enum class ShadowResolutionSetting { None = 0, Low = 1, Medium = 2, High = 3 };

enum class AmbientOcclusionTypeSetting { None = 0, GTAO = 1 };

enum class AmbientOcclusionQualitySetting { None = 0, High = 1, Ultra = 2 };

enum class SSRQualitySetting { Off = 0, Medium = 1, High = 2 };
...
struct RendererTieringSettings {
  ...
  AmbientOcclusionQualitySetting AOQuality =
      AmbientOcclusionQualitySetting::Ultra;
...
inline const char* ShadowQualitySettingToString(
    ShadowQualitySetting shadowQualitySetting) {
  switch (shadowQualitySetting) {
    case ShadowQualitySetting::None:
      return "None";
...
inline ShadowQualitySetting ShadowQualitySettingFromString(
    std::string_view shadowQualitySetting) {
...
inline const char* ShadowResolutionSettingToString(
    ShadowResolutionSetting shadowResolutionSetting) {
  switch (shadowResolutionSetting) {
    case ShadowResolutionSetting::None:
      return "None";
...
inline ShadowResolutionSetting ShadowResolutionSettingFromString(
    std::string_view shadowResolutionSetting) {
  ...
  if (shadowResolutionSetting == "Medium")
    return ShadowResolutionSetting::Medium;
  if (shadowResolutionSetting == "High") return ShadowResolutionSetting::High;

  return ShadowResolutionSetting::None;
}

inline const char* AmbientOcclusionQualitySettingToString(
...
inline AmbientOcclusionQualitySetting AmbientOcclusionQualitySettingFromString(
    std::string_view ambientOcclusionQualitySetting) {
  if (ambientOcclusionQualitySetting == "None")
    return AmbientOcclusionQualitySetting::None;
  if (ambientOcclusionQualitySetting == "Low")
    return AmbientOcclusionQualitySetting::High;  // NOTE(Yan): low has been
                                                  // renamed to high, currently
                                                  // there is no low
  if (ambientOcclusionQualitySetting == "High")
    return AmbientOcclusionQualitySetting::High;
  if (ambientOcclusionQualitySetting == "Ultra")
    return AmbientOcclusionQualitySetting::Ultra;

  return AmbientOcclusionQualitySetting::None;
}

inline const char* SSRQualitySettingToString(
    SSRQualitySetting ssrQualitySetting) {
...
inline SSRQualitySetting SSRQualitySettingFromString(
    std::string_view ssrQualitySetting) {
