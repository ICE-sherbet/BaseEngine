...
inline Matrix33 mat3_cast(Quaternion const& q) {
  ...
  Result[0][0] = Floating(1) - Floating(2) * (qyy + qzz);
  Result[0][1] = Floating(2) * (qxy + qwz);
  Result[0][2] = Floating(2) * (qxz - qwy);

  Result[1][0] = Floating(2) * (qxy - qwz);
  Result[1][1] = Floating(1) - Floating(2) * (qxx + qzz);
  Result[1][2] = Floating(2) * (qyz + qwx);

  Result[2][0] = Floating(2) * (qxz + qwy);
  Result[2][1] = Floating(2) * (qyz - qwx);
  Result[2][2] = Floating(1) - Floating(2) * (qxx + qyy);
  return Result;
}

inline Matrix44 mat4_cast(Quaternion const& q) {
	const Matrix44 mat = glm::mat4_cast(q);
  return mat;
}
