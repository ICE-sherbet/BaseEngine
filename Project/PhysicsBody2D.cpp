...
PhysicsFixture* PhysicsBody::CreateFixture(const PhysicsFixtureDef* def) {
  ...
  PhysicsBlockAllocator* allocator = &m_world->block_allocator_;
  ...
  if (m_flags & e_enabledFlag) {
    bp::BroadPhase* broadPhase = &m_world->contact_manager_.m_broadPhase;
...
void PhysicsBody::DestroyFixture(PhysicsFixture* fixture) {
  ...
  while (edge) {
    ...
    if (fixture == fixtureA || fixture == fixtureB) {
      ...
      m_world->contact_manager_.Destroy(c);
    }
  }
...
void PhysicsBody::SetTransform(const PVec2& position, const float angle)
{
  if (m_xf.p == position) {
    return;
  }
